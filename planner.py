...
def get_neighbors(node):
    ...
    # action_weights = {(0, RPM1): 1.1, (RPM1, 0): 1.1, (RPM1, RPM1): 1, (0, RPM2): 1.2, (RPM2, 0): 1.2, (RPM2, RPM2): 1.1, (RPM1, RPM2): 1.3, (RPM2, RPM1): 1.3}
...
def reconstruct_path(came_from, start, goal):
    current = goal
    path_with_velocities = [(current, came_from[current][1] if current in came_from else (0, 0))] # Initialize the path with velocities
    while current != start:
...
def visualize_path(path):
    ...
    for i in range(len(path) - 1):
        ...
        for linear_vel, angular_vel in velocities:
            V.append((linear_vel/100, -angular_vel)) # Extract the linear and angular velocities and convert to m/s
            tn = t
            xa = x_new
            ya = y_new
            ...
            tn += angular_vel * 0.1
            cv2.line(canvas, (int(round(xa)), int(round(ya))), (int(round(x_new)), int(round(y_new)),), (0, 0, 255), 1) # Draw the path on the canvas
            canvas_resized = cv2.resize(canvas, (1200, 400), interpolation=cv2.INTER_AREA)
            out.write(canvas_resized)
        cv2.line(canvas, (int(round(x_new)), int(round(y_new))), (int(round(xn)), int(round(yn)),), (0, 0, 255), 1)
